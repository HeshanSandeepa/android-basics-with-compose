...
fun CategoryList(
    ...
    onEmailCardPressed: (Category) -> Unit,
    modifier: Modifier = Modifier
) {
    val categories = replyUiState.categories
    ...
    LazyColumn(
        ...
        items(categories, key = {category -> category.id}) {category ->
            CategoryItem(category = category, selected = false,         onCardClick = {
                onEmailCardPressed(category)
            })
        }
    }
}
...
@Composable
private fun CategoryTopBar(modifier: Modifier = Modifier) {
    Row(
